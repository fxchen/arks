...
- **kb** ([github.com/hraness/kb](https://github.com/hraness/kb), MIT) —
  Ben Guo's knowledge-base CLI, whose philosophy closely matches ARKS:
  Markdown and git stay authoritative, indexes are disposable. Its read and
  search commands worked against an ARKS tree via `--root` when tested
  against v0.17 (2026-08).
  Notes for ARKS use: skip `kb init` (this template replaces it), set
  `kb_catalog: authored` so it writes nothing, and use its read and search
  commands only. Before pointing any kb capture feature at `Records/`, verify
  its output is a single conforming Record file.
  Honest caveats: young project, single maintainer, requires the Bun runtime —
  have your agent do the install.
- **qmd** ([github.com/tobi/qmd](https://github.com/tobi/qmd)) — local
  ...
  (about 2 GB). If you adopt it, configure its ignore list so `AGENTS.md` and
  other instruction files stay out of the index (engine principle 5).
